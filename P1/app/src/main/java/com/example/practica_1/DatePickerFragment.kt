...
class DatePickerFragment(val listener : (year : Int, month : Int, day : Int ) -> Unit): DialogFragment(), DatePickerDialog.OnDateSetListener  {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if(currentDate.isNotEmpty()){
            val fechaActualFormateada = DateTimeFormat.forPattern(Constantes.defaultPatternFecha).parseDateTime(currentDate)
        ...
        if(maxDate.isNotEmpty()){
            val fechaMaximaFormateada = DateTimeFormat.forPattern(Constantes.defaultPatternFecha).parseDateTime(maxDate)
        ...
        if(minDate.isNotEmpty()){
            val fechaMinimaFormateada = DateTimeFormat.forPattern(Constantes.defaultPatternFecha).parseDateTime(minDate)
